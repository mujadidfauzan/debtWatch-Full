...
const AddDebtDialog: React.FC<AddDebtDialogProps> = ({ isOpen, onClose, onAddDebt, onEditDebt, initialData, onDeleteDebt }) => {
  ...
  useEffect(() => {
    if (initialData && isOpen) {
      ...
      setTotalCicilan(initialData.totalCicilan?.toString() || '');
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const commonData = {
      namaUtang,
      totalCicilan: parseInt(totalCicilan, 10) || 0,
    ...
    if (isEditMode && onEditDebt && initialData) {
      onEditDebt({ ...commonData, id: initialData.id });
    } else if (!isEditMode && onAddDebt) {
      onAddDebt(commonData);
    }
  };

  const handleDelete = () => {
    if (initialData && initialData.id && onDeleteDebt) {
      if (window.confirm(`Apakah Anda yakin ingin menghapus utang "${initialData.namaUtang}"?`)) {
        onDeleteDebt(initialData.id);
      }
    }
  };
      ...
      <div className="bg-white p-6 rounded-lg shadow-xl w-full max-w-md">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-semibold text-gray-800">
            {isEditMode ? 'Edit Utang' : 'Tambah Utang Baru'}
          </h2>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit}>
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label htmlFor="totalCicilan" className="block text-sm font-medium text-gray-700 mb-1">
                  Total Cicilan (Bulan)
                </label>
                <input
                  type="number"
                  ...
                  required
                  value={totalCicilan}
                  onChange={(e) => setTotalCicilan(e.target.value)}
                  min="1"
                  className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
