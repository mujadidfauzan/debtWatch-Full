...
const Question_Utang: React.FC = () => {
  ...
  const handleSaveAndNext = async () => {
    ...
    try {
      ...
      console.error('Gagal menyimpan data utang:', error);
      alert('Terjadi kesalahan saat menyimpan data utang.');
    }
  };
    ...
    <div className="bg-blue-700 min-h-screen p-4 flex flex-col items-center text-white">
      <div className="w-full max-w-2xl p-6 rounded-lg">
        ...
        <div className="bg-gray-200 text-black rounded-b-lg max-h-[50vh] overflow-y-auto">
          {utangItems.length === 0 && <p className="p-4 text-center text-gray-500">Belum ada data utang. Silakan tambahkan.</p>}
          {utangItems.map((item, index) => (
            <div key={item.id} className={`grid grid-cols-12 gap-x-2 p-3 items-center text-sm ${index < utangItems.length - 1 ? 'border-b border-gray-300' : ''}`}>
              <input type="text" value={item.nama} onChange={(e) => handleUtangItemChange(item.id, 'nama', e.target.value)} placeholder="KPR, Mobil, dll." className="col-span-3 p-2 border rounded bg-white" />
              ...
              <input
                ...
                min="0"
              />
              <button onClick={() => handleRemoveUtang(item.id)} className="col-span-1 text-red-500 hover:text-red-700 p-1 rounded-full flex items-center justify-center" title="Hapus Utang">
