...
import { getCategoriesByType } from '@/lib/categoryUtils';
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gradient-to-b from-app-blue to-blue-600">
      <div className="p-4 flex-1 flex flex-col">
        ...
        <div className="mb-4">
          <AmountDisplay amount={amount} firstOperand={firstOperand} operation={operation} waitingForSecondOperand={waitingForSecondOperand} />
        </div>

        <div className="space-y-3 mb-4">
          <Popover>
            <PopoverTrigger asChild>
              <Button variant={'outline'} className={cn('w-full justify-start text-left font-normal bg-white/20 border-white/50 text-white hover:text-white', !selectedDate && 'text-muted-foreground')}>
          ...
          <Input type="text" placeholder="Write a note... (optional)" value={note} onChange={(e) => setNote(e.target.value)} className="w-full p-2 rounded bg-white/20 text-white placeholder-white" />
        </div>

        <Button onClick={handleAddTransaction} disabled={!isFormValid || mutation.isPending} className="w-full bg-yellow-400 hover:bg-yellow-500 text-black font-semibold py-3 rounded-lg mb-4 disabled:opacity-50">
